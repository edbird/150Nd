        ...
        if(paramNameToNumberMap.count(search_object) > 0)
        {
            ...
            if(std::find(enabled_params.begin(), enabled_params.end(), param_number) != enabled_params.end())
            {
                ...
                if(gDirectory->GetListOfKeys()->Contains(theHistogram + BkgFiles[i] + "_fit"))
                {
                    ...
                    tmpHist = (TH1F*)gDirectory->Get(theHistogram + BkgFiles[i] + "_fit")->Clone(BkgFiles[i] + "_" + theChannel + thePhase);
                ...
                {
                    std::cout << "gDirectory->GetListOfKeys() does not contain " << theHistogram + BkgFiles[i] + "_fit" << std::endl;
                }
            }
            else
            {
...
void book1DHistograms(Int_t channel_counter, TString theChannel, TString thePhase, TString theHistogram) {
    ...
    book1DHistograms_helper(channel_counter, theChannel, thePhase, theHistogram, nExternalBkgs_oneParam, ExternalBkgOneParamFiles, tmpHist);
    /*
    TH1F *tmpHist  = new TH1F("tmpHist_" + theChannel + thePhase, "" , 1, 0, 1);
    for(int i = 0; i < nExternalBkgs_oneParam; i++)
    {
        ...
        //std::cout << "searching for string " << ExternalBkgOneParamFiles[i] << " in paramNameMap" << std::endl;
        //TString search_object = ExternalBkgOneParamFiles[i];
        std::string mc_name = std::string(ExternalBkgOneParamFiles[i]);
        std::string search_object = MCNameToParamNameMap.at(mc_name);
        if(paramNameToNumberMap.count(search_object) > 0)
        {
            int param_number = paramNameToNumberMap.at(search_object);
    ...
    */
    ...
    std::cout << "print out paramNameToNumberMap" << std::endl;
    for(auto it = paramNameToNumberMap.cbegin(); it != paramNameToNumberMap.cend(); ++ it)
    {
        std::cout << it->first << " -> " << it->second << std::endl;
    }
    */
    ...
    book1DHistograms_helper(channel_counter, theChannel, thePhase, theHistogram, nInternalBkgs, InternalBkgFiles, tmpHist);
    /*
    for(int i = 0; i < nInternalBkgs; i++)
    {
        // check if parameter is enabled
        // convert parameter string name to index
        //std::cout << "searching for string " << InternalBkgFiles[i] << " in paramNameMap" << std::endl;
        //TString search_object = InternalBkgFiles[i];
        //std::string search_object = std::string(InternalBkgFiles[i]);
        std::string mc_name = std::string(InternalBkgFiles[i]);
        std::string search_object = MCNameToParamNameMap.at(mc_name);
        if(paramNameToNumberMap.count(search_object) > 0)
        {
            int param_number = paramNameToNumberMap.at(search_object);
    ...
    */
    ...
    book1DHistograms_helper(channel_counter, theChannel, thePhase, theHistogram, nRn222Bkgs, Rn222BkgFiles, tmpHist);
    /*
    for(int i = 0; i < nRn222Bkgs; i++)
    {
        // check if parameter is enabled
        // convert parameter string name to index
        //std::cout << "searching for string " << Rn222BkgFiles[i] << " in paramNameMap" << std::endl;
        //TString search_object = Rn222BkgFiles[i];
        //std::string search_object = std::string(Rn222BkgFiles[i]);
        std::string mc_name = std::string(Rn222BkgFiles[i]);
        std::string search_object = MCNameToParamNameMap.at(mc_name);
        if(paramNameToNumberMap.count(search_object) > 0)
        {
            int param_number = paramNameToNumberMap.at(search_object);
    ...
    */
    ...
    book1DHistograms_helper(channel_counter, theChannel, thePhase, theHistogram, nRn220Bkgs, Rn220BkgFiles, tmpHist);
    /*
    for(int i = 0; i < nRn220Bkgs; i++)
    {
        // check if parameter is enabled
        // convert parameter string name to index
        //std::cout << "searching for string " << Rn220BkgFiles[i] << " in paramNameMap" << std::endl;
        //TString search_object = Rn220BkgFiles[i];
        //std::string search_object = std::string(Rn220BkgFiles[i]);
        std::string mc_name = std::string(Rn220BkgFiles[i]);
        std::string search_object = MCNameToParamNameMap.at(mc_name);
        if(paramNameToNumberMap.count(search_object) > 0)
        {
            int param_number = paramNameToNumberMap.at(search_object);
    ...
    */
    ...
    book1DHistograms_helper(channel_counter, theChannel, thePhase, theHistogram, nNd150Samples, Nd150Files, tmpHist);
    /*
    for(int i = 0; i < nNd150Samples; i++)
    {
        // check if parameter is enabled
        // convert parameter string name to index
        //std::cout << "searching for string " << Nd150Files[i] << " in paramNameMap" << std::endl;
        //TString search_object = Nd150Files[i];
        //std::string search_object = std::string(Nd150Files[i]);
        std::string mc_name = std::string(Nd150Files[i]);
        std::string search_object = MCNameToParamNameMap.at(mc_name);
        if(paramNameToNumberMap.count(search_object) > 0)
        {
            int param_number = paramNameToNumberMap.at(search_object);
    ...
    */
    ...
    book1DHistograms_helper(channel_counter, theChannel, thePhase, theHistogram, nNeighbours, NeighbourFiles, tmpHist);
    /*
    for(int i = 0; i < nNeighbours; i++)
    {
        // check if parameter is enabled
        // convert parameter string name to index
        //std::cout << "searching for string " << NeighbourFiles[i] << " in paramNameMap" << std::endl;
        //TString search_object = NeighbourFiles[i];
        //std::string search_object = std::string(NeighbourFiles[i]);
        std::string mc_name = std::string(NeighbourFiles[i]);
        std::string search_object = MCNameToParamNameMap.at(mc_name);
        if(paramNameToNumberMap.count(search_object) > 0)
        {
            int param_number = paramNameToNumberMap.at(search_object);
...
void fitBackgrounds(double *AdjustActs, double *AdjustActs_Err, Int_t thePhase)
{
    ...
    */
    ...
    std::cout << "list of free/fixed parameters" << std::endl;
