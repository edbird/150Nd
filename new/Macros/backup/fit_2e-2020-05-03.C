...
void makeHistograms(TString thePath, TString sampleName, std::ofstream &ofile_cutcount);
...
void loadFiles()
{
    ...
    makeHistograms("betabeta/", DataFile, ofile_cutcount);
    ...
    {
        makeHistograms("externals/", ExternalBkgFiles[i], ofile_cutcount);
    }
    std::cout << "... DONE." << std::endl;
    ...
    {
        makeHistograms("externals/", Rn220BkgFiles[i], ofile_cutcount);
    }
    std::cout << "... DONE." << std::endl;
    ...
    {
        makeHistograms("externals/", Rn222BkgFiles[i], ofile_cutcount);
    }
    std::cout << "... DONE." << std::endl;
    ...
    {
        makeHistograms("internals/", InternalBkgFiles[i], ofile_cutcount);
    }
    std::cout << "... DONE." << std::endl;
    ...
    {
        makeHistograms("neighbours/", NeighbourFiles[i], ofile_cutcount);
    }
    std::cout << "... DONE." << std::endl;
    ...
    {
        makeHistograms("nd150/", Nd150Files[i], ofile_cutcount);
    }
    std::cout << "... DONE." << std::endl;
...
void makeHistograms(TString thePath, TString sampleName, std::ofstream& ofile_cutcount)
{
    ...
    hRun                    = new TH1F("hRun_" + sampleName,
                                       "Phase " + Phase + " " + sampleName + " runs; run numbers",
                                       8000, 0, 10000); // 1000, 8000

    hNElectrons             = new TH1F("hNElectrons_" + sampleName,
                                       "Phase " + Phase + " " + sampleName + " N tracks; N tracks/event",
                                       20, -0.5, 19.5); // limits? all events are 2 for nd150

    hNAPromptGgHits         = new TH1F("hNAPromptGgHits_" + sampleName,
                                       "Phase " + Phase + " " + sampleName + " N unassoc. prompt gg hits; N unassoc prompt gg hits",
                                       20, -0.5, 19.5); // limits? blank histogram for nd150?

    hTotalE                 = new TH1F("hTotalE_" + sampleName,
                                       "Phase " + Phase + " " + sampleName + " total energy; #SigmaE_{e} (MeV)",
                                       50, 0.0, 4.0); // 0.0, 2.2

    hInternalPullee         = new TH1F("hInternalPullee_" + sampleName,
                                       "Phase " + Phase + " " + sampleName + " internal hypothesis; ee Pull",
                                       50, -40., 40.); // limits? all events within -10, 10 for nd150

    hInternalProbee         = new TH1F("hInternalProbee_" + sampleName,
                                       "Phase " + Phase + " " + sampleName + " internal hypothesis; ee Probability",
                                       50, 0.0, 1.); // limits?

    hExternalPullee         = new TH1F("hExternalPullee_" + sampleName,
                                       "Phase " + Phase + " " + sampleName + " external hypothesis; ee Pull",
                                       50, -40., 40.); // limits? all events within -30, 10 for nd150

    hExternalProbee         = new TH1F("hExternalProbee_" + sampleName,
                                       "Phase " + Phase + " " + sampleName + " external hypothesis; ee Probability",
                                       50, 0.0, 1.); // limits?

    hCosee                  = new TH1F("hCosee_" + sampleName,
                                       "Phase " + Phase + " " + sampleName + " angular correlation of electron tracks; cos(#theta)_{ee}",
                                       50, -1., 1.); // limits?

    hCoseeWeighted          = new TH1F("hCoseeWeighted_" + sampleName,
                                       "Phase " + Phase + " " + sampleName + "corrected angular correlation of electron tracks; cos(#theta)_{ee}",
                                       50, -1., 1.); // limits?

    hVertexDZ               = new TH1F("hVertexDZ_" + sampleName,
                                       "Phase " + Phase + " " + sampleName + "distance between recon vertices; #DeltaZ (cm)",
                                       50, -20., 20.); // limits? all events within -10, 10 for nd150

    hVertexDR               = new TH1F("hVertexDR_" + sampleName,
                                       "Phase " + Phase + " " + sampleName + "distance between recon vertices; #DeltaR (cm)",
                                       50, -1., 1.); // limits?

    hVertexDRPhi            = new TH1F("hVertexDRPhi_" + sampleName,
                                       "Phase " + Phase + " " + sampleName + "distance between recon vertices; #DeltaR#phi (cm*rad)",
                                       50,-10.,10.); // limits? all events witnin -10, 10 for nd150

    hEeMax                  = new TH1F("hEeMax_" + sampleName,
                                       "Phase " + Phase + " " + sampleName + " higher energy Ee; E_{e} (MeV)"              ,
                                       50, 0.0, 4.0); // limits ok

    hElectronLengthMax      = new TH1F("hElectronLengthMax_" + sampleName,
                                       "Phase " + Phase + " " + sampleName + " higher energy e^{-} track length;  track length (cm)",
                                       50, 0, 600); // limits?

    hVertexZMax             = new TH1F("hVertexZMax_" + sampleName,
                                       "Phase " + Phase + " " + sampleName + " higher energy vertex position;  Z (cm)"                  ,
                                       50, -120, 120); // limits?
                                       //50, -150, 150); // limits?

    hVertexSectorMax        = new TH1F("hVertexSectorMax_" + sampleName,
                                       "Phase " + Phase + " " + sampleName + " higher energy vertex position;  sector"                 ,
                                       50, 5.7, 5.9); // limits?
                                       //50, 5.5, 6.1); // limits?

    hVertexRMax             = new TH1F("hVertexRMax_" + sampleName,
                                       "Phase " + Phase + " " + sampleName + " higher energy vertex position;  R (cm)"                  ,
                                       //50, 154.7, 155.0); // limits? all events within 154.85, 154.9 for nd150
                                       50, 154.8900, 154.8912); // limits? all events within 154.85, 154.9 for nd150

    hElectronFirstGgMax     = new TH1F("hElectronFirstGgMax_" + sampleName,
                                       "Phase " + Phase + " " + sampleName + " higher energy e^{-} first gg hit location;first gg layer",
                                       9, -0.5, 8.5); // all in 0 for nd150

    hElectronLastGgMax      = new TH1F("hElectronLastGgMax_" + sampleName,
                                       "Phase " + Phase + " " + sampleName + " higher energy e^{-} last gg hit location;last gg layer",
                                       9, -0.5, 8.5); // all in 7/8 for nd150

    hVertexMinDistPromptGgMax  = new TH1F("hVertexMinDistPromptGgMax_" + sampleName,
                                       "Phase " + Phase + " " + sampleName + " distance from higher energy vertex to closest, prompt unassoc. gg hit;(v_{x}, v_{y}, v_{z}) - (gg_{x},gg_{y},gg_{z}) (cm)",
                                       50, 0, 600); // blank?

    hElectronLDCorrMax      = new TH1F("hElectronLDCorrMax_" + sampleName,
                                       "Phase "+Phase+" "+sampleName+" higher energy e^{-} LD correction;LD corr", 
                                       50, 0.9,1.1); // limits? don't know what this histogram is


    hVertexZSecMax          = new TH2F("hVertexZSecMax_" + sampleName,
                                       "Phase "+Phase+" "+sampleName+" higher energy vertex location; sector; Z (cm)",
                                       100, 5.7, 5.9,
                                       //100, 5.5, 6.1,
                                       100, -120, 120); 

    hEeMin                  = new TH1F("hEeMin_" + sampleName,
                                       "Phase "+Phase+" "+sampleName+" lower energy Ee; E_{e} (MeV)",
                                       50, 0.0, 4);

    hElectronLengthMin      = new TH1F("hElectronLengthMin_" + sampleName,
                                       "Phase "+Phase+" "+sampleName+" lower energy e^{-} track length;  track length (cm)",
                                       50, 0, 600);

    hVertexZMin             = new TH1F("hVertexZMin_" + sampleName,
                                       "Phase "+Phase+" "+sampleName+" lower energy vertex position;  Z (cm)"                  ,
                                       50, -120, 120);
                                       //50, -150, 150);

    hVertexSectorMin        = new TH1F("hVertexSectorMin_" + sampleName,
                                       "Phase "+Phase+" "+sampleName+" lower energy vertex position;  sector"                 ,
                                       50, 5.7, 5.9);
                                       //50, 5.5, 6.1);

    hVertexRMin             = new TH1F("hVertexRMin_" + sampleName,
                                       "Phase "+Phase+" "+sampleName+" lower energy vertex position;  R (cm)"                  ,
                                       //50, 154.7, 155.0);
                                       50, 154.8904, 154.8907);

    hElectronFirstGgMin     = new TH1F("hElectronFirstGgMin_" + sampleName,
                                       "Phase "+Phase+" "+sampleName+" lower energy e^{-} first gg hit location;first gg layer",
                                       9, -0.5, 8.5);

    hElectronLastGgMin      = new TH1F("hElectronLastGgMin_" + sampleName,
                                       "Phase "+Phase+" "+sampleName+" lower energy e^{-} last gg hit location;last gg layer",
                                       9, -0.5, 8.5);

    hVertexMinDistPromptGgMin   = new TH1F("hVertexMinDistPromptGgMin_" + sampleName,
                                           "Phase " + Phase + " " + sampleName + " distance from lower energy vertex to closest, prompt unassoc. gg hit;(v_{x},v_{y},v_{z}) - (gg_{x},gg_{y},gg_{z}) (cm)",
                                           50, 0, 600);

    hElectronLDCorrMin      = new TH1F("hElectronLDCorrMin_" + sampleName,
                                       "Phase "+Phase+" "+sampleName+" lower energy e^{-} LD correction;LD corr"            ,
                                       50, 0.9, 1.1);

    hVertexZSecMin          = new TH2F("hVertexZSecMin_" + sampleName,
                                       "Phase "+Phase+" "+sampleName+" lower energy vertex location; sector; Z (cm)"              ,
                                       150, 5.8904, 5.8907,
                                       100, -120, 120);


    hNLowEGammas            = new TH1F("hNLowEGammas_" + sampleName,
                                       "Phase "+Phase+" "+sampleName+" unassoc. scint hits E < 200 keV; N hits"     ,
                                       11, -0.5, 10.5);

    hLowEGammaEnergy        = new TH1F("hLowEGammaEnergy_" + sampleName,
                                       "Phase "+Phase+" "+sampleName+" unassoc. scint hits E < 200 keV; Energy (indiv. hits) (MeV)" ,
                                       50, 0, 0.2);

    hSummedLowEGammaE       = new TH1F("hSummedLowEGammaE_" + sampleName,
                                       "Phase "+Phase+" "+sampleName+" unassoc. scint hits E < 200 keV; Energy (indiv. hits) (MeV)" ,
                                       50, 0, 0.5);

    hLowEMinDistPromptGg    = new TH1F("hLowEMinDistPromptGg_" + sampleName,
                                       "Phase "+Phase+" "+sampleName+" distance from unassoc. calo hits to closest, prompt unassoc. gg hit;(v_{x},v_{y},v_{z}) - (gg_{x},gg_{y},gg_{z}) (cm)",
                                       100, 0, 600.);//all low E hits


    hEeMaxVEeMin = new TH2F("hEeMaxVEeMin_" + sampleName,
                                       "Phase " + Phase + " " + sampleName + " electron energies; Ee^{Max} (MeV); Ee^{min} (MeV)",
                                       50, 0, 4, 50, 0, 4);

    hNAPromptGgHitsDist2VertexMin = new TH1F("hNAPromptGgHitsDist2VertexMin_" + sampleName,
                                       "Phase " + Phase + " " + sampleName + " min vertex distance to unassociated prompt geiger hit; min vertex distance (cm); y label",
                                       50, 0., 100.);
                                        



    hNAfterCuts      = new TH1F("hNAfterCuts_" + sampleName,
                                       "Phase " + Phase + " " + sampleName + " Analysis Cut Flow; ; N events",
                                       50, 0, 50); //never know how many we'll make... :)

    hEffAfterCuts      = new TH1F("hEffAfterCuts_" + sampleName,
                                       "Phase " + Phase + " " + sampleName + " Analysis Cut Flow; ; efficiency",
                                       50, 0, 50); //never know how many we'll make... :)

    hTrackSignMax   = new TH1F("hTrackSignMax_" + sampleName,
                                "Phase " + Phase + " " + sampleName + " Analysis Cut Flow; ; Track Sign (Max)",
                                21, -10, 10);

    hTrackSignMin   = new TH1F("hTrackSignMin_" + sampleName,
                                "Phase " + Phase + " " + sampleName + " Analysis Cut Flow; ; Track Sign (Min)",
                                21, -10, 10);
    ...
    histogramPointers["hRun_"] = hRun;
    histogramPointers["hNElectrons_"] = hNElectrons;
    histogramPointers["hTotalE_"] = hTotalE;
    histogramPointers["hInternalPullee_"] = hInternalPullee;
    histogramPointers["hInternalProbee_"] = hInternalProbee;
    histogramPointers["hExternalPullee_"] = hExternalPullee;
    histogramPointers["hExternalProbee_"] = hExternalProbee;
    histogramPointers["hCosee_"] = hCosee;
    histogramPointers["hCoseeWeighted_"] = hCoseeWeighted;
    histogramPointers["hVertexDZ_"] = hVertexDZ;
    histogramPointers["hVertexDR_"] = hVertexDR;
    histogramPointers["hVertexDRPhi_"] = hVertexDRPhi;
    histogramPointers["hNAPromptGgHits_"] = hNAPromptGgHits;
    histogramPointers["hEeMax_"] = hEeMax;
    histogramPointers["hElectronLengthMax_"] = hElectronLengthMax;
    histogramPointers["hVertexZMax_"] = hVertexZMax;
    histogramPointers["hVertexSectorMax_"] = hVertexSectorMax;
    histogramPointers["hVertexRMax_"] = hVertexRMax;
    histogramPointers["hElectronFirstGgMax_"] = hElectronFirstGgMax;
    histogramPointers["hElectronLastGgMax_"] = hElectronLastGgMax;
    histogramPointers["hVertexMinDistPromptGgMax_"] = hVertexMinDistPromptGgMax;
    histogramPointers["hElectronLDCorrMax_"] = hElectronLDCorrMax;
    histogramPointers["hVertexZSecMax_"] = hVertexZSecMax;
    histogramPointers["hEeMin_"] = hEeMin;
    histogramPointers["hElectronLengthMin_"] = hElectronLengthMin;
    histogramPointers["hVertexZMin_"] = hVertexZMin;
    histogramPointers["hVertexSectorMin_"] = hVertexSectorMin;
    histogramPointers["hVertexRMin_"] = hVertexRMin;
    histogramPointers["hElectronFirstGgMin_"] = hElectronFirstGgMin;
    histogramPointers["hElectronLastGgMin_"] = hElectronLastGgMin;
    histogramPointers["hVertexMinDistPromptGgMin_"] = hVertexMinDistPromptGgMin;
    histogramPointers["hElectronLDCorrMin_"] = hElectronLDCorrMin;
    histogramPointers["hVertexZSecMin_"] = hVertexZSecMin;
    histogramPointers["hNLowEGammas_"] = hNLowEGammas;
    histogramPointers["hLowEGammaEnergy_"] = hLowEGammaEnergy;
    histogramPointers["hSummedLowEGammaE_"] = hSummedLowEGammaE;
    histogramPointers["hLowEMinDistPromptGg_"] = hLowEMinDistPromptGg;
    histogramPointers["hEeMaxVEeMin_"] = hEeMaxVEeMin;
    histogramPointers["hNAfterCuts_"] = hNAfterCuts;
    histogramPointers["hEffAfterCuts_"] = hEffAfterCuts;
    histogramPointers["hNAPromptGgHitsDist2VertexMin_"] = hNAPromptGgHitsDist2VertexMin;
    histogramPointers["hTrackSignMax_"] = hTrackSignMax;
    histogramPointers["hTrackSignMin_"] = hTrackSignMin;
...
*/
    ...
    std::cout << "writing histograms into \"rawdata" << "\'" << std::endl;
    std::cout << "writing: rawdata ... " << hTotalE->GetName() << std::endl;
    ...
    for(int i = 0; i < numHistograms; ++ i)
    {
        ...
        histogramPointers.at(histogram_name)->Write();
    }
