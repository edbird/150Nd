...
*/
        ...
        hInternalPullee->Fill(eeInternalPull,1);
        hInternalProbee->Fill(eeInternalProb,1);
        hExternalPullee->Fill(eeExternalPull,1);
        hExternalProbee->Fill(eeExternalProb,1);
...
void scale(TFile* myFile, const int nData, TString *dataFiles, TString *dataNames, std::string typedir, double *dataNGenMC, std::string activityfile, double *dataActivity, double *dataEfficiency, Color_t *dataColor, Color_t *dataColors, TObjArray** allData, double *AcceptedTime, double additional_scaling_factor)
{
    ...
    for(int i = 0; i < nData; i++)
    {
        ...
        //inFile2.open("../include/internalActivities.txt");
        inFile2.open(activityfile.c_str());
        double tmpP1, tmpP2;
        TString dummy2;
        while(!inFile2.eof())
        {
            inFile2 >> dummy2 >> tmpP1 >> tmpP2;
            // cout << "Read in from activity file: " << dummy2
            // 	   << ", " << tmpP1
            // 	   << ", " << tmpP2 << endl;
            // cout << "Trying to compare to " << InternalBkgFiles[i] << endl;
            if(dummy2.CompareTo(dataFiles[i]) == 0)
            {
                //dataActivity[0][i] = tmpP1;
                //dataActivity[1][i] = tmpP2;
                dataActivity[0 * nData + i] = tmpP1;
                dataActivity[1 * nData + i] = tmpP2;
            }
        }
        inFile2.close();
        ...
        for(int j = 0; j < numHistograms; j++)
        {
            ...
            if(j == 0)
            {
                ...
                dataEfficiency[thePhase * nData + i] = eff;
            }
            tmpHist->Sumw2();
            ...
            tmpHist->Scale(dataActivity[thePhase * nData + i] * AcceptedTime[thePhase] * additional_scaling_factor / (dataNGenMC[i] * (AcceptedTime[thePhase]/(double)TotalTime)) );
            ...
            oftemp << dataFiles[i] << " -> " << dataNames[i] << " : " << histogramNames[j] << " -> sf=" << dataActivity[thePhase * nData + i] * AcceptedTime[thePhase] * additional_scaling_factor / (dataNGenMC[i] * (AcceptedTime[thePhase]/(double)TotalTime)) << ", " << dataActivity[thePhase * nData + i] << " integral()=> " << tmpHist->Integral() << std::endl;
...
#define SCALE 1
    ...
    std::ofstream oftemp("oftemp2.txt", std::ofstream::out);

    //Let's get the internals and add them to an array
    TObjArray *allInternals[nInternalBkgs];
    for(int i = 0; i < nInternalBkgs; i++)
    {
        std::cout << InternalBkgFiles[i] << std::endl;
        allInternals[i] = new TObjArray();

        //read in the information needed for calculating efficiencies
        std::ifstream inFile;
        inFile.open(filePath+"internals/"+InternalBkgFiles[i]+"/JobSummary.txt");
        ...
        std::string dummy;
        inFile >> dummy >> InternalBkgNGenMC[i];
        // cout << "Read in as dummy: " << dummy << endl;
        // cout << "Read in as InternalBkgNGenMC[i]: " << InternalBkgNGenMC[i] << endl;
        inFile.close();
        ...
        while(!inFile2.eof())
        {
            ...
            // 	   << ", " << tmpP2 << endl;
            // cout << "Trying to compare to " << InternalBkgFiles[i] << endl;
            if(dummy2.CompareTo(InternalBkgFiles[i]) == 0)
            {
                InternalBkgActivity[0][i] = tmpP1;
                InternalBkgActivity[1][i] = tmpP2;
            }
        }
        inFile2.close();
        ...
        for(int j = 0; j < numHistograms; j++)
        {
            tmpHist = (TH1F*)myFile->Get("singleHistos/unscaled/"+histogramNames[j]+InternalBkgFiles[i])->Clone(histogramNames[j]+InternalBkgFiles[i]+"_fit");
            // cout << "The histogram I have got is "
            // 	   << "singleHistos/unscaled/"
            // 	   << histogramNames[j]
            // 	   << InternalBkgFiles[i]
            ...
            if(j == 0)
            {
                ...
                double eff = NPass / (double)(InternalBkgNGenMC[i] * (AcceptedTime[thePhase]/(double)TotalTime) );
                std::cout << "eff: " << eff << std::endl;

                InternalBkgEfficiency[thePhase][i] = eff;
            }
            tmpHist->Sumw2();
            tmpHist->SetFillColor(InternalBkgColors[i]);
            tmpHist->SetLineColor(InternalBkgColors[i]);
            tmpHist->SetTitle(InternalBkgNames[i]);
            // cout << "Integral before scaling: " << tmpHist->Integral() << endl;
            // cout << "InternalBkgActivity[thePhase][i]: " << InternalBkgActivity[thePhase][i] << endl
            // 	   << "AcceptedTime[thePhase]: " << AcceptedTime[thePhase] << endl
            // 	   << "InternalBkgNGenMC[i]: " << InternalBkgNGenMC[i] << endl
            ...
            // 	   << "TotalTime: " << TotalTime << endl;
            tmpHist->Scale(InternalBkgActivity[thePhase][i] * AcceptedTime[thePhase] * 0.001 / (InternalBkgNGenMC[i] * (AcceptedTime[thePhase]/(double)TotalTime)) );
            //std::cout << "sf=" << InternalBkgActivity[thePhase][i] * AcceptedTime[thePhase] * 0.001 / (InternalBkgNGenMC[i] * (AcceptedTime[thePhase]/(double)TotalTime)) << std::endl;
            oftemp << InternalBkgNames[i] << " -> sf=" << InternalBkgActivity[thePhase][i] * AcceptedTime[thePhase] * 0.001 / (InternalBkgNGenMC[i] * (AcceptedTime[thePhase]/(double)TotalTime)) << ", " << InternalBkgActivity[thePhase][i] << std::endl;
            // cout << "Integral after scaling: " << tmpHist->Integral() << endl;

            allInternals[i]->Add(tmpHist);
    ...
    //Let's get the neighbour samples and add them to an array
    TObjArray *allNeighbours[nNeighbours];
    for (int i = 0; i < nNeighbours; i++)
    {
        std::cout <<NeighbourFiles[i] << std::endl;
        allNeighbours[i] = new TObjArray(); 

        //read in the information needed for calculating efficiencies
        std::ifstream inFile;
        inFile.open(filePath+"neighbours/"+NeighbourFiles[i]+"/JobSummary.txt");
        std::string dummy;
        inFile >> dummy >> NeighbourNGenMC[i];
        inFile.close();

        //if we want to use a pre-measured background activity (which we do for the time being) then we
        //have to scale the histogram appropriately.  So now we need to fetch the activity from the txt file
        std::ifstream inFile2;
        inFile2.open("../include/internalActivities2.txt");
        double tmpP1, tmpP2;
        TString dummy2;
        while(!inFile2.eof())
        {
            ...
            if(dummy2.CompareTo(NeighbourFiles[i]) == 0)
            {
                NeighbourActivity[0][i] = tmpP1;
                NeighbourActivity[1][i] = tmpP2;
            }
        }
        inFile2.close();

        // cout << "***************************************************************" << endl;
        // cout << "Next up is NeighbourActivity: " << NeighbourActivity[0][i]
        // 	 << " and " << NeighbourActivity[1][i]
        // 	 << endl;

        std::cout << "A: " << NeighbourActivity[thePhase][i] << std::endl;

        for(int j = 0; j < numHistograms; j++)
        {
            tmpHist = (TH1F*)myFile->Get("singleHistos/unscaled/"+histogramNames[j]+NeighbourFiles[i])->Clone(histogramNames[j]+NeighbourFiles[i]+"_fit");
            //calculate efficiency
            if( j == 0 )
            {
                double NPass = tmpHist->Integral();
                std::cout << "NPass: " << NPass << std::endl;
                double eff = NPass / (double)(NeighbourNGenMC[i] * (AcceptedTime[thePhase]/(double)TotalTime) );
                std::cout << "eff: " << eff << std::endl;

                NeighbourEfficiency[thePhase][i] = eff;
            }
            tmpHist->Sumw2();
            tmpHist->SetFillColor(NeighbourColor[thePhase]);
            tmpHist->SetLineColor(NeighbourColor[thePhase]);
            tmpHist->SetTitle(NeighbourNames[i]);
            tmpHist->Scale(NeighbourActivity[thePhase][i] * AcceptedTime[thePhase] * 0.001 / (NeighbourNGenMC[i] * (AcceptedTime[thePhase]/(double)TotalTime)) );
            //std::cout << "sf=" << NeighbourActivity[thePhase][i] * AcceptedTime[thePhase] * 0.001 / (NeighbourNGenMC[i] * (AcceptedTime[thePhase]/(double)TotalTime)) << std::endl;
            oftemp << NeighbourNames[i] << " -> sf=" << NeighbourActivity[thePhase][i] * AcceptedTime[thePhase] * 0.001 / (NeighbourNGenMC[i] * (AcceptedTime[thePhase]/(double)TotalTime)) << ", " << NeighbourActivity[thePhase][i] << std::endl;

            allNeighbours[i]->Add(tmpHist);
        }//all histos
    }//nd150

    //Let's get the nd150 samples and add them to an array
    TObjArray *allNd150Samples[nNd150Samples];
    for(int i = 0; i < nNd150Samples; i++)
    {
        std::cout <<Nd150Files[i] << std::endl;
        allNd150Samples[i] = new TObjArray(); 

        //read in the information needed for calculating efficiencies
        std::ifstream inFile;
        inFile.open(filePath+"nd150/"+Nd150Files[i]+"/JobSummary.txt");
        std::string dummy;
        inFile >> dummy >> Nd150NGenMC[i];
        inFile.close();
        ...
        {
            inFile2 >> dummy2 >> tmpP1 >> tmpP2;
            if(dummy2.CompareTo(Nd150Files[i]) == 0)
            {
                Nd150Activity[0][i] = tmpP1;
                Nd150Activity[1][i] = tmpP2;
            }
        }
        inFile2.close();
        ...
        // cout << "***************************************************************" << endl;
        // cout << "Next up is Nd150Activity: " << Nd150Activity[0][i]
        // 	 << " and " << Nd150Activity[1][i]
        // 	 << endl;

        std::cout << "A: " << Nd150Activity[thePhase][i] << std::endl;

        for(int j = 0; j < numHistograms; j++)
        {
            tmpHist = (TH1F*)myFile->Get("singleHistos/unscaled/"+histogramNames[j]+Nd150Files[i])->Clone(histogramNames[j]+Nd150Files[i]+"_fit");
            //calculate efficiency
            if( j == 0 )
            {
                double NPass = tmpHist->Integral();
                std::cout << "NPass: " << NPass << std::endl;
                double eff = NPass / (double)(Nd150NGenMC[i] * (AcceptedTime[thePhase]/(double)TotalTime) );
                std::cout << "eff: " << eff << std::endl;

                Nd150Efficiency[thePhase][i] = eff;
            }
            tmpHist->Sumw2();
            tmpHist->SetFillColor(Nd150BkgColor[thePhase]);
            tmpHist->SetLineColor(Nd150BkgColor[thePhase]);
            tmpHist->SetTitle(Nd150Names[i]);
            tmpHist->Scale(Nd150Activity[thePhase][i] * AcceptedTime[thePhase] * 0.001 / (Nd150NGenMC[i] * (AcceptedTime[thePhase]/(double)TotalTime)) );
            //std::cout << "sf=" << Nd150Activity[thePhase][i] * AcceptedTime[thePhase] * 0.001 / (Nd150NGenMC[i] * (AcceptedTime[thePhase]/(double)TotalTime)) << std::endl;
            oftemp << Nd150Names[i] << " -> sf=" << Nd150Activity[thePhase][i] * AcceptedTime[thePhase] * 0.001 / (Nd150NGenMC[i] * (AcceptedTime[thePhase]/(double)TotalTime)) << ", " << Nd150Activity[thePhase][i] << std::endl;

            allNd150Samples[i]->Add(tmpHist);
        }//all histos
    }//nd150


    //Let's get the externals and add them to an array
    TObjArray *allExternals[nExternalBkgs];
    for (int i = 0; i < nExternalBkgs; i++)
    {
        std::cout <<ExternalBkgFiles[i] << std::endl;
        allExternals[i] = new TObjArray();

        std::ifstream inFile;
        inFile.open(filePath+"externals/"+ExternalBkgFiles[i]+"/JobSummary.txt");
        std::string dummy;
        inFile >> dummy >> ExternalBkgNGenMC[i];
        inFile.close();
        //std::cout << ExternalBkgNGenMC[i] << std::endl;

        //if we want to use a pre-measured background activity (which we do for the time being) then we
        //have to scale the histogram appropriately.  So now we need to fetch the activity from the txt file
        std::ifstream inFile2;
        inFile2.open("../include/externalActivities.txt");
        double tmpP1, tmpP2;
        TString dummy2;
        while(!inFile2.eof())
        {
            inFile2 >> dummy2 >> tmpP1 >> tmpP2;
            if(dummy2.CompareTo(ExternalBkgFiles[i]) == 0)
            {
                ExternalBkgActivity[0][i] = tmpP1;
                ExternalBkgActivity[1][i] = tmpP2;
            }
        }
        inFile2.close();

        // cout << "***************************************************************" << endl;
        // cout << "Next up is ExternalBkgActivity: " << ExternalBkgActivity[0][i]
        // 	 << " and " << ExternalBkgActivity[1][i]
        // 	 << endl;


        for(int j = 0; j < numHistograms; j++)
        {
            tmpHist = (TH1F*)myFile->Get("singleHistos/unscaled/"+histogramNames[j]+ExternalBkgFiles[i])->Clone(histogramNames[j]+ExternalBkgFiles[i]+"_fit");
            ...
            if( j == 0 )
            {
                double NPass = tmpHist->Integral();
                //	std::cout << "NPass: " << NPass << std::endl;
                double eff = NPass / (ExternalBkgNGenMC[i] * (AcceptedTime[thePhase]/(double)TotalTime) );
                //	std::cout << "eff: " << eff << std::endl;

                ExternalBkgEfficiency[thePhase][i] = eff;
            }
            tmpHist->Sumw2();
            tmpHist->SetFillColor(ExternalBkgColor[thePhase]);
            tmpHist->SetLineColor(ExternalBkgColor[thePhase]);
            tmpHist->Scale(ExternalBkgActivity[thePhase][i] * AcceptedTime[thePhase] / (ExternalBkgNGenMC[i] * (AcceptedTime[thePhase]/(double)TotalTime)) );
            //std::cout << "sf=" << ExternalBkgActivity[thePhase][i] * AcceptedTime[thePhase] / (ExternalBkgNGenMC[i] * (AcceptedTime[thePhase]/(double)TotalTime)) << std::endl;
            oftemp << ExternalBkgNames[i] << " -> sf=" << ExternalBkgActivity[thePhase][i] * AcceptedTime[thePhase] / (ExternalBkgNGenMC[i] * (AcceptedTime[thePhase]/(double)TotalTime)) << ", " << ExternalBkgActivity[thePhase][i] << std::endl;

            allExternals[i]->Add(tmpHist);
        }//all histos
    }//externals
    ...
    for(int i = 0; i < nRn222Bkgs; i++)
    {
        std::cout <<Rn222BkgFiles[i] << std::endl;
        allRn222Bkgs[i] = new TObjArray();

        std::ifstream inFile;
        inFile.open(filePath+"externals/"+Rn222BkgFiles[i]+"/JobSummary.txt");
        std::string dummy;
        inFile >> dummy >> Rn222BkgNGenMC[i];
        inFile.close();

        std::cout << Rn222BkgNGenMC[i] << std::endl;
        //if we want to use a pre-measured background activity (which we do for the time being) then we
        //have to scale the histogram appropriately.  So now we need to fetch the activity from the txt file
        std::ifstream inFile2;
        inFile2.open("../include/radonActivities.txt");
        double tmpP1, tmpP2;
        TString dummy2;
        while(!inFile2.eof())
        {
            inFile2 >> dummy2 >> tmpP1 >> tmpP2;
            if(dummy2.CompareTo(Rn222BkgFiles[i]) == 0)
            {
                Rn222BkgActivity[0][i] = tmpP1;
                Rn222BkgActivity[1][i] = tmpP2;
            }
        }
        inFile2.close();
        ...
        // cout << "***************************************************************" << endl;
        // cout << "Next up is Rn222BkgActivity: " << Rn222BkgActivity[0][i]
        // 	 << " and " << Rn222BkgActivity[1][i]
        // 	 << endl;

        for(int j = 0; j < numHistograms; j++)
        {
            tmpHist = (TH1F*)myFile->Get("singleHistos/unscaled/"+histogramNames[j]+Rn222BkgFiles[i])->Clone(histogramNames[j]+Rn222BkgFiles[i]+"_fit");
            ...
            if(j == 0)
            {
                double NPass = tmpHist->Integral();
                //	std::cout << "NPass: " << NPass << std::endl;
                double eff = NPass / (Rn222BkgNGenMC[i] * (AcceptedTime[thePhase]/(double)TotalTime) );
                //	std::cout << "eff: " << eff << std::endl;

                Rn222BkgEfficiency[thePhase][i] = eff;
            }
            tmpHist->Sumw2();
            tmpHist->SetFillColor(Rn222BkgColor[thePhase]);
            tmpHist->SetLineColor(Rn222BkgColor[thePhase]);
            tmpHist->Scale(Rn222BkgActivity[thePhase][i] * AcceptedTime[thePhase] / (Rn222BkgNGenMC[i] *  (AcceptedTime[thePhase]/(double)TotalTime) ) );
            //std::cout << "sf=" << Rn222BkgActivity[thePhase][i] * AcceptedTime[thePhase] / (Rn222BkgNGenMC[i] *  (AcceptedTime[thePhase]/(double)TotalTime) ) << std::endl;
            oftemp << Rn222BkgNames[i] << " -> sf=" << Rn222BkgActivity[thePhase][i] * AcceptedTime[thePhase] / (Rn222BkgNGenMC[i] *  (AcceptedTime[thePhase]/(double)TotalTime) ) << ", " << Rn222BkgActivity[thePhase][i] << std::endl;

            allRn222Bkgs[i]->Add(tmpHist);
    ...
    for (int i = 0; i < nRn220Bkgs; i++)
    { 
        std::cout <<Rn220BkgFiles[i] << std::endl;
        allRn220Bkgs[i] = new TObjArray();   // one array of all histograms for each sample 

        std::ifstream inFile;
        inFile.open(filePath+"externals/"+Rn220BkgFiles[i]+"/JobSummary.txt");
        std::string dummy;
        inFile >> dummy >> Rn220BkgNGenMC[i];
        inFile.close();
        ...
        {
            inFile2 >> dummy2 >> tmpP1 >> tmpP2;
            if(dummy2.CompareTo(Rn220BkgFiles[i]) == 0)
            {
                Rn220BkgActivity[0][i] = tmpP1;
                Rn220BkgActivity[1][i] = tmpP2;
            }
        }
        inFile2.close();

        // cout << "***************************************************************" << endl;
        // cout << "Next up are some Rn220BkgActivities: " << Rn220BkgActivity[0][i]
        // 	 << " and " << Rn220BkgActivity[1][i]
        // 	 << endl;


        for(int j = 0; j < numHistograms; j++)
        {
            tmpHist = (TH1F*)myFile->Get("singleHistos/unscaled/"+histogramNames[j]+Rn220BkgFiles[i])->Clone(histogramNames[j]+Rn220BkgFiles[i]+"_fit");
            ...
            if(j == 0)
            {
                double NPass = tmpHist->Integral();
                //	std::cout << "NPass: " << NPass << std::endl;
                double eff = NPass / (Rn220BkgNGenMC[i] * (AcceptedTime[thePhase]/(double)TotalTime) );
                //	std::cout << "eff: " << eff << std::endl;

                Rn220BkgEfficiency[thePhase][i] = eff;
            }
            tmpHist->Sumw2();
            tmpHist->SetFillColor(Rn220BkgColor[thePhase]);
            tmpHist->SetLineColor(Rn220BkgColor[thePhase]);
            tmpHist->Scale(Rn220BkgActivity[thePhase][i] * AcceptedTime[thePhase] / (Rn220BkgNGenMC[i] * (AcceptedTime[thePhase]/(double)TotalTime)) );
            //std::cout << "sf=" << Rn220BkgActivity[thePhase][i] * AcceptedTime[thePhase] / (Rn220BkgNGenMC[i] * (AcceptedTime[thePhase]/(double)TotalTime)) << std::endl;
            oftemp << Rn220BkgNames[i] << " -> sf=" << Rn220BkgActivity[thePhase][i] * AcceptedTime[thePhase] / (Rn220BkgNGenMC[i] * (AcceptedTime[thePhase]/(double)TotalTime)) << ", " << Rn220BkgActivity[thePhase][i] << std::endl;

            allRn220Bkgs[i]->Add(tmpHist);

        }//all histos
    }//Rn220
    // tmpHist->Delete(); // no use any more
    ...
    TH1F *hAllExt[numHistograms];
    TH1F *hAllRn[numHistograms];
    TH1F *hAllInt[numHistograms];
    ...
    TH1F *allMC[numHistograms]; //for all MC samples to calculate chi2
    ...
    for(int i = 0; i < numHistograms-1; i++)
    {
        ...
        TString events;
        events.Form("%i",(int)hAllExt[i]->Integral());
        hAllExt[i]->SetTitle("Externals ("+events+")");

        events.Form("%i",(int)hAllRn[i]->Integral());
        hAllRn[i]->SetTitle("Radon ("+events+")");

        events.Form("%i",(int)hAllInt[i]->Integral());
        hAllInt[i]->SetTitle("Internals ("+events+")");
        ...
        hStacks[i]->Add((TH1F*)hAllExt[i]);
        hStacks[i]->Add((TH1F*)hAllRn[i]);
        hStacks[i]->Add((TH1F*)hAllInt[i]);
    ...
    TH2F *allMC2D[numHistograms]; //for all MC samples to calculate chi2
    TH2F *tmp2D;
    for(int i = numHistograms-1; i < numHistograms; i++)
    {
        ...
        for( int j = 0; j < nExternalBkgs; j++)
        {
            ...
            if(j == 0)
            {
                allMC2D[i] = (TH2F*)allExternals[j]->At(i)->Clone("allMC"+histogramNames[i]);
            }
            else
            {
                allMC2D[i]->Add((TH2F*)allExternals[j]->At(i)->Clone());
            }
        }//externals
        ...
        for(int j = 0; j < nRn222Bkgs; j++)
        {
            ...
            allMC2D[i]->Add((TH2F*)allRn222Bkgs[j]->At(i)->Clone());
        }//radon222

        for(int j = 0; j < nRn220Bkgs; j++)
        {
            (TH2F*)allRn220Bkgs[j]->At(i)->Write();
            allMC2D[i]->Add((TH2F*)allRn220Bkgs[j]->At(i)->Clone());
        }//rn220

        for(int j = 0; j < nInternalBkgs; j++)
        {
            (TH2F*)allInternals[j]->At(i)->Write();
            allMC2D[i]->Add((TH2F*)allInternals[j]->At(i)->Clone());
        }//internals

        for(int j = 0; j < nNeighbours; j++)
        {
            (TH2F*)allNeighbours[j]->At(i)->Write();
            allMC2D[i]->Add((TH2F*)allNeighbours[j]->At(i)->Clone());
        }//internals

        for(int j = 0; j < nNd150Samples; j++)
        {
            (TH2F*)allNd150Samples[j]->At(i)->Write();
            allMC2D[i]->Add((TH2F*)allNd150Samples[j]->At(i)->Clone());
        }//internals
