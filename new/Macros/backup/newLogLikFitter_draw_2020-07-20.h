    ...
    int draw_index = -1)
{
    ...
    for(int channel = 0; channel < allDataSamples1D->GetEntries(); channel)
    {
        ...
        data1D[channel] = (TH1D*)allDataSamples1D->At(channel)->Clone();
        if(mode_fake_data == true)
        {
            fakeData1D[channel] = (TH1D*)allFakeDataSamples1D->At(channel)->Clone(); // TODO
        ...
        c[channel] = new TCanvas("c" + channel_str, "c" + channel_str);
        c[channel]->SetFillColor(kWhite);

        
        stacks1D[i] = new THStack("stacks1D" + channel_str, channel_str);
        ...
        stacks1D_major[i]->GetYaxis()->SetTitle("Events / 0.1 MeV");
